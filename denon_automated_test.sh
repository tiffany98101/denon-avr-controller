...
run_cmd "help includes data fields commands" bash -c 'unset DENON_PROFILE; source "$1"; out=$(denon help); [[ "$out" == *"denon data fields --all"* && "$out" == *"denon data dump --json"* && "$out" == *"read-only sources"* && "$out" == *"DENON_DATA_DISCOVERY_MAX_TYPE"* ]]' bash "$SCRIPT"
run_cmd "data fields --all runs offline" bash -c 'unset DENON_PROFILE DENON_IP DENON_DEFAULT_IP; source "$1"; out=$(denon data fields --all); [[ "$out" == *"Main Zone"* && "$out" == *"Raw XML endpoints"* ]]' bash "$SCRIPT"
run_cmd "data fields --all includes expected fields" bash -c 'unset DENON_PROFILE; source "$1"; out=$(denon data fields --all); [[ "$out" == *"Known read-only/exposed"* && "$out" == *"power"* && "$out" == *"source_name"* && "$out" == *"volume_db"* && "$out" == *"muted"* && "$out" == *"zone2_sources"* && "$out" == *"type_12"* ]]' bash "$SCRIPT"
run_cmd "data usage rejects invalid option" bash -c 'unset DENON_PROFILE; source "$1"; denon data fields --bogus >/dev/null 2>&1; [[ "$?" -ne 0 ]]' bash "$SCRIPT"
run_cmd "data raw endpoint catalog includes type 3 4 6 7 12" bash -c 'unset DENON_PROFILE; source "$1"; out=$(_denon_data_field_catalog); [[ "$out" == *$'\''type_3'\''* && "$out" == *$'\''type_4'\''* && "$out" == *$'\''type_6'\''* && "$out" == *$'\''type_7'\''* && "$out" == *$'\''type_12'\''* ]]' bash "$SCRIPT"
run_cmd "data XML leaf parser extracts nested leaves" bash -c 'unset DENON_PROFILE; source "$1"; denon help >/dev/null; xml="<Root><MainZone><Power>1</Power><Nested><Value>abc</Value></Nested></MainZone></Root>"; out=$(_denon_data_xml_leaf_paths "$xml"); [[ "$out" == *$'\''Root.MainZone.Power\t1'\''* && "$out" == *$'\''Root.MainZone.Nested.Value\tabc'\''* ]]' bash "$SCRIPT"
run_cmd "data web information parser extracts versions" bash -c 'unset DENON_PROFILE; source "$1"; denon help >/dev/null; html="<table><tr><td>Firmware Version</td><td>1.2.3</td></tr><tr><td>DTS Version</td><td>4.5.6</td></tr></table>"; out=$(_denon_data_parse_web_information "$html"); [[ "$out" == *$'\''Firmware Version\t1.2.3'\''* && "$out" == *$'\''DTS Version\t4.5.6'\''* ]]' bash "$SCRIPT"
run_cmd "data web discovery keeps safe endpoints only" bash -c 'unset DENON_PROFILE; source "$1"; denon help >/dev/null; text=$'\''"/ajax/globals/get_config?type=3" "/ajax/globals/set_config?type=4" "/setup/reset.html" "/general/info.js"'\''; out=$(_denon_data_discover_web_endpoints_from_text "$text"); [[ "$out" == *"/ajax/globals/get_config?type=3"* && "$out" == *"/general/info.js"* && "$out" != *"set_config"* && "$out" != *"reset"* ]]' bash "$SCRIPT"
run_cmd "data JSON printer escapes values" bash -c 'unset DENON_PROFILE; source "$1"; denon help >/dev/null; data_available_records=$'\''receiver\tReceiver\tname\tDenon "Test"\n'\''; data_source_rows_main=""; data_source_rows_zone2=""; data_get_config_types=""; data_get_config_leaf_records=""; data_discovered_endpoint_records=""; out=$(_denon_data_print_json); [[ "$out" == *"Denon \\\"Test\\\""* ]]' bash "$SCRIPT"
